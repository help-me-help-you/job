...
class ActionHandler {
	public function __construct() {
		AnnotationRegistry::registerAutoloadNamespace( 'ListingManager\Annotation\Action', LISTING_MANAGER_DIR . '/src/' );
	}
				...
				if ( is_array( $method_annotations ) ) {
					foreach ( $method_annotations as $annotation ) {
						if ( $annotation instanceof Action ) {
							add_action( $annotation->name, [ $class_name, $method ], $annotation->priority, $annotation->accepted_args );
						}
					}
				}
			}
